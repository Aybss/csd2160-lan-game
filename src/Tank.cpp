...
static constexpr float SHOOT_CD = 0.5f;  // seconds between shots
                  ...
                  const std::vector<Obstacle>& obs)
{
    ...
    float dx=0, dy=0;
    if(fwd) { dx += cosf(rad)*TANK_SPEED*dt; dy += sinf(rad)*TANK_SPEED*dt; }
    if(back){ dx -= cosf(rad)*TANK_SPEED*dt; dy -= sinf(rad)*TANK_SPEED*dt; }
